...
# ---------------------------------------------------------------------------
# Step 5: Rebuild versions.js (loaded as <script> by index.html — works offline)
# ---------------------------------------------------------------------------

def rebuild_versions_js(output_dir):
    output_dir = Path(output_dir)
    json_files = sorted(output_dir.glob('sponsors_*.json'), reverse=True)

    if not json_files:
        print("  WARNING: No sponsor JSON files found in data/")
        return
    ...
    for jf in json_files:
        ...
        versions_meta.append({
            "date": v,
            "label": d.get("label", format_label(v)),
            "foi": d['source'].get('foi_reference', ''),
            "total": d['total'],
        })
        versions_data[v] = d['sponsors']

    db = {"versions": versions_meta, "data": versions_data}
    js = json.dumps(db, ensure_ascii=False, separators=(',', ':'))

    js_path = output_dir / 'versions.js'
    with open(js_path, 'w', encoding='utf-8') as f:
        f.write('// Auto-generated by extract.py — do not edit manually\n')
        f.write(f'window.SPONSOR_DB={js};\n')

    print(f"  Updated: {js_path}  ({len(versions_meta)} version(s) total)")
...
def main():
    ...
    rebuild_versions_js(args.output)
